// Called from [id].js, by clicking apply
// create a form that collects a coverletter field.
// when a user clicks on apply to this job, we make a post to /api/application
...
export default function Apply({ job }) {
    ...
    const handleSubmit = async (event) => {
        ...
        // Post the content to the API route /api/setup for the user name 
            ...
            try{
                const result = await fetch('/api/application', requestParams)
            }catch(error){
                console.error(error)
            }
